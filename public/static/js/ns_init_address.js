...
function initProvince(obj){
	$.ajax({
		type : "post",
		url : "http://www.jayhui.cn/home/order/getProvince",
		dataType : "json",
		success : function(data) {
			if (data != null && data.length > 0) {
				var str = "";
				for (var i = 0; i < data.length; i++) {
					str += '<option value="'+data[i].id+'">'+data[i].region_name+'</option>';
				}
				$(obj).append(str);
			}
		}
...
function getProvince(obj,second,city_id) {
	...
	$.ajax({
		type : "post",
		url : "http://www.jayhui.cn/home/order/getCity",
		dataType : "json",
		data : {
			"province_id" : id
		},
			...
			if (data != null && data.length > 0) {
				...
				for (var i = 0; i < data.length; i++) {
					if(city_id ==data[i].city_id){
						str += '<option value="'+data[i].id+'" selected="selected">'+data[i].region_name+'</option>';
					}else{
						str += '<option value="'+data[i].id+'">'+data[i].region_name+'</option>';
					}
				}
...
function getSelCity(obj,second,district_id) {
	...
	$.ajax({
		type : "post",
		url : "http://www.jayhui.cn/home/order/getDistrict",
		dataType : "json",
		data : {
			"city_id" : id
		},
