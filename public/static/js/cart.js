...
$(".js-clear-cart").click(function() {
	$.ajax({
		url : "http://www.jayhui.cn/home/cart/emptycart",
		type : "get",
		datatype:"json",
...
function add_num(cart_id) {
	...
	//}
	var num = parseInt($("#goods_number_6").val());
	//获取商品的价格
	var price = $("#subtotal_6").attr("data-price");
	num++;
	$.ajax({
		url:"http://www.jayhui.cn/home/cart/changeNum",
		type:"post",
		...
		success:function(data)
		{
			if (data.code > 0) {
				...
				var sum = price * num;
				//设置商品的数量
				$("#goods_number_6").val(num);
				//设置商品的总额
				$("#subtotal_6").html("¥"+sum);
				$("#cart_money_info span").html("¥"+sum);
			}
		}
	});
}
...
function minus_num(cart_id, max_buy, stock) {
	...
	//获取商品的数量
	var num = parseInt($("#goods_number_"+cart_id).val());
	//获取商品的价格
	var price = $("#subtotal_"+cart_id).attr("data-price");
	//当商品的数量大于1才可以减
	if (num > 1) {
		num--;
		$.ajax({
			...
			success:function(data)
			{
				if (data.code > 0) {
					...
					var sum = price * num;
					//设置商品的数量
					$("#goods_number_"+cart_id).val(num);
					//设置商品的总额
					$("#subtotal_"+cart_id).html("¥"+sum);
					var total = parseInt($("#cart_money_info").attr("data-total"))+price;
					$("#cart_money_info span").html("¥"+total);
				}
